      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginTop:5}}>
            ...
            <span style={{color:"#0a1228",fontSize:9,fontFamily:"'Space Grotesk',sans-serif",fontWeight:400}}>
              {rem}/{total}
            </span>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <span style={{fontFamily:"'Libre Baskerville',serif",fontSize:22,color:"#0a1228"}}>
            {price.toLocaleString()} €
          </span>
          <button onClick={()=>setDone(!done)}
            style={{background:done?"#0a1228":"transparent",border:"1px solid #0a1228",
              color:done?"#ffffff":"#0a1228",padding:"7px 18px",fontSize:9,letterSpacing:2,
...
function ContactForm({t}){
  ...
  const valid=name.trim()&&email.trim()&&msg.trim()&&consent;
  const onSend=(e)=>{
    e.preventDefault();
    if(!valid) return;
    const subject=`Contact — ${name}`;
    const body=`${t.n1}: ${name}\n${t.n2}: ${email}\n\n${msg}`;
    window.location.href=`mailto:smoreu@mac.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  };
  ...
  return(
    <form onSubmit={onSend} style={{display:"flex",flexDirection:"column",gap:9,
      ...
      <textarea value={msg} onChange={e=>setMsg(e.target.value)}
        placeholder={t.n3} rows={5} style={{...inp,resize:"vertical"}} required/>
      <label style={{display:"flex",alignItems:"flex-start",gap:8,fontSize:11,
        color:"#0a1228",fontFamily:"'Space Grotesk',sans-serif",fontWeight:400,
        lineHeight:1.5,cursor:"pointer",marginTop:4}}>
        <input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}
          style={{marginTop:3,accentColor:"#0a1228"}} required/>
        <span>{t.siRgpd}</span>
      </label>
      <button type="submit" disabled={!valid} className="bg"
        style={{width:"auto",display:"inline-block",alignSelf:"center",marginTop:6,
          opacity:valid?1:0.4,cursor:valid?"pointer":"not-allowed"}}>
        {t.ns} ✉
      </button>
    </form>
  );
          ...
          <div className="fade-in" style={{position:"fixed",top:"calc(14px + env(safe-area-inset-top,0px))",
            ...
            <button onClick={()=>setLangOpen(!langOpen)}
              style={{background:"#ffffff",border:"1px solid #0a1228",
                color:"#0a1228",padding:"4px 8px",fontSize:18,lineHeight:1,
                ...
                {LANGS.filter(l=>l!==lang).map(l=>(
                  <button key={l}
                    ...
                    style={{display:"block",width:"100%",background:"none",border:"none",
                      borderBottom:"1px solid #ffffff",padding:"8px 0",fontSize:18,lineHeight:1,
                      cursor:"pointer",fontFamily:"'Apple Color Emoji','Segoe UI Emoji','Noto Color Emoji',sans-serif",
            ...
            {PRINTS.map((p,idx)=>(
              <div key={p.id} className="hs"
                ...
                <div style={{flex:1,padding:"20px 18px",display:"flex",
                  ...
                  <p style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:400,
                    fontSize:10,color:"#0a1228",lineHeight:1.5}}>{p.tech}</p>
                  <p style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:400,
      ...
      {sec==="contact"&&(
        <div style={{maxWidth:500,margin:"60px auto",padding:"0 18px 70px",textAlign:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:32,
            ...
            <button onClick={()=>goSec("portfolio")}
              style={{background:"none",border:"none",cursor:"pointer",
                color:"#0a1228",fontSize:18,lineHeight:1}}>←</button>
            <Logo sz={48}/>
          </div>
          <h2 style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:500,fontSize:18,
            letterSpacing:5,marginBottom:4}}>A.V.S.M PRINTS</h2>
          <p style={{color:"#0a1228",fontSize:9,letterSpacing:2,marginBottom:28,
            fontFamily:"'Space Grotesk',sans-serif",fontWeight:400}}>
            I LOVE YOU MONEYPENIS
          </p>
          <ContactForm t={t}/>
          <div style={{marginTop:30,paddingTop:20,borderTop:"1px solid #0a1228",
            textAlign:"left"}}>
            <p style={{color:"#0a1228",fontSize:11,marginBottom:12,lineHeight:1.7,
              fontFamily:"'Space Grotesk',sans-serif",fontWeight:400,fontStyle:"italic"}}>
              {t.siPro}
            </p>
            <p style={{color:"#0a1228",fontSize:10,lineHeight:1.7,
